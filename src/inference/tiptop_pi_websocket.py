"""Tiptop websocket client with pi0.5 fallback.

Connects to tiptop server, gets a plan, executes it. After the plan finishes,
calls a VLM to check task completion. If the VLM says the task is not complete,
falls back to pi0.5 for closed-loop recovery.
"""
...
from .droid_jointpos import Client as Pi05Client
...
class TiptopPiWebsocketClient(InferenceClient):
    """Queries tiptop server for a plan, executes it, then falls back to pi0.5 if VLM says task incomplete."""
    ...
    def __init__(
        ...
        self._pi05_port = pi05_port
        self._fallback_client: Optional[Pi05Client] = None
    ...
    def infer(self, obs: dict, instruction: str) -> dict:
        # If we've already switched to pi0.5, delegate entirely
        if self._fallback_client is not None:
            return self._fallback_client.infer(obs, instruction)
    ...
    def _plan_exhausted(self) -> bool:
        """True when all plan steps have been consumed."""
        if self._plan is None:
            return True
        if self._current_plan_step >= len(self._plan):
            # Also make sure there's no pending gripper action
